...
from dataLoadForMyModel import trainloaderForPhotoA, trainloaderForSketchA, testloaderForPhotoA, testloaderForSketchA
...
class Digit(nn.Module):
    def __init__(self, labelNum):
        ...
        self.conv1_2 = nn.Sequential(
            nn.Conv2d(10, 10, 5, padding=2),  # 3：图片输入通道为3，10：输出通道，5：5*5卷积核
            nn.BatchNorm2d(10),  # 批归一化
            nn.LeakyReLU()  # 激活函数
        )
        # 二维卷积层2_1
        ...
        self.conv2_2 = nn.Sequential(
            nn.Conv2d(20, 20, 3, padding=1),  # 10：输入通道，20：输出通道，3：3*3卷积核
            nn.BatchNorm2d(20),  # 批归一化
            nn.Dropout(p=0.2),  # 使神经元随机失效
            nn.LeakyReLU()
        )
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(20, 10, 3, padding=1),  # 20：输入通道，20：输出通道，3：3*3卷积核
            nn.BatchNorm2d(10),  # 批归一化
            nn.Dropout(p=0.2),  # 使神经元随机失效
            nn.LeakyReLU()
        )
    ...
    def forward(self, x, labelNum):
        ...
        x = self.conv1_1(x)  # 输入：batch_size*3*256*256，输出：batch_size*10*256*256（其中256=256-5+1+4）
        x = self.conv1_2(x)  # 输入：batch_size*10*256*256，输出：batch_size*10*256*256（其中256=256-5+1+4）
        x = F.max_pool2d(x, 2, 2)  # 池化层（数据压缩，降采样），输入：batch_size*10*256*256，输出：batch_size*10*128*128

        x = self.conv2_1(x)  # 输入：batch_size*10*128*128，输出：batch_size*20*128*128（其中128=128-3+1+2）
        x = self.conv2_2(x)  # 输入：batch_size*20*128*128，输出：batch_size*20*128*128（其中128=128-3+1+2）
        ...
        x = self.conv4(x)  # 输入：batch_size*20*64*64，输出：batch_size*10*64*64（其中64=64-3+1+2）

        x = x.view(input_size, -1)  # 降维到1维，-1：自动计算原始维度

        x = self.fc1(x)  # 输入：batch_size*10*64*64，输出：batch_size*5000
...
def trainAndTest(labelNo, type, result):
    for epoch in range(1, EPOCHS + 1):
        ...
        if type == 'photo':
            train_model(model, DEVICE, trainloaderForPhotoA, optimizer, epoch, labelNo)
            correct = test_model(model, DEVICE, testloaderForPhotoA, labelNo)
            if epoch == EPOCHS:
                result.append({'数据类型': 'photo', '分类用的标签': labels[labelNo], '分类正确率': correct})
        elif type == 'sketch':
            train_model(model, DEVICE, trainloaderForSketchA, optimizer, epoch, labelNo)
            correct = test_model(model, DEVICE, testloaderForSketchA, labelNo)
            if epoch == EPOCHS:
